...
export default function SubmitPage() {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setGuestInfo({ name: user.name || "", email: user.email || "", phone: user.phone || "" });
    }
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (storedUser) {
        ...
        const { user: guest } = await authRes.json();
        userId = guest.id;
      }
      ...
      const subRes = await fetch("/api/submissions", {
        ...
        body: JSON.stringify({
          userId,
          trackUrl,
          trackType: platform, // Add platform type
          planId: selectedPlan,
          ...
          guestName: !storedUser ? guestInfo.name : undefined,
          guestEmail: !storedUser ? guestInfo.email : undefined,
        }),
      });
      ...
      console.error(error);
      setErrorMessage("Something went wrong with your submission.");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <main className="flex-1 pt-20">
        <section className="py-8 border-b border-gold/10">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <Link href="/" className="inline-flex items-center gap-2 text-luxury-gray hover:text-gold mb-4 transition-colors">
            ...
            <p className="text-luxury-gray text-left">Start your promotion journey in just a few steps</p>
          </div>
        </section>
                    ...
                    <div className="space-y-4 pt-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div className="space-y-2">
                            ...
                            <Input type="email" value={guestInfo.email} onChange={e => setGuestInfo({...guestInfo, email: e.target.value})} className="bg-luxury-lighter border-gold/20" />
                          </div>
                        </div>
                        <div className="space-y-2">
                            <Label className="text-white">Phone (Optional)</Label>
                            <Input value={guestInfo.phone} onChange={e => setGuestInfo({...guestInfo, phone: e.target.value})} className="bg-luxury-lighter border-gold/20" />
                        </div>
